...
shinyServer(function(input, output, session) {
    ...
    # DT Options --------------------------------------------------------------
    options(DT.options = list(lengthMenu = c(10, 20), dom = 'tl'))  # table and lengthMenu options
    
    # Intro JS ----------------------------------------------------------------
    observeEvent(input$help, introjs(
        session,
        options = list(
            "nextLabel" = "Next",
            "prevLabel" = "Back",
    ...
    observeEvent(input$btn1, {
        if (values$data == 1) {
            req(input$item_name)
            values$frozen_job1 <- input$item_name
            values$data <- 2
            
        } else if (values$data == 2) {
            req(input$select2_rows_selected)
            # Lock in the selection from Table 1 before it disappears
            values$frozen_job2 <- top1()[input$select2_rows_selected, "Item2Name"]
            values$data <- 3
            
        } else if (values$data == 3) {
            req(input$select3_rows_selected)
            # Lock in the selection from Table 2 before it disappears
            values$frozen_job3 <- top2()[input$select3_rows_selected, "Item2Name"]
            values$data <- 4
        } else if (values$data == 4) {
            req(input$select4_rows_selected)
            # Lock in the selection from Table 2 before it disappears
            values$frozen_job4 <- top3()[input$select4_rows_selected, "Item2Name"]
            values$data <- 5
        } else if (values$data == 5) {
            req(input$select5_rows_selected)
            # Lock in the selection from Table 2 before it disappears
            values$frozen_job4 <- top4()[input$select5_rows_selected, "Item2Name"]
            # values$data <- 5
        }
    })
    ...
    observeEvent(input$goBack, {
        ...
        if (values$data == 5 &
            !is.null(input$select5_rows_selected)) {
            showModal(
                modalDialog(
                    "Please remove your selection before going back.",
                    size = "s",
                    title = "Oops!",
                   ...
                   !is.null(input$select4_rows_selected)) {
            showModal(
                modalDialog(
                    "Please remove your selection before going back.",
                    size = "s",
                   ...
                   !is.null(input$select3_rows_selected)) {
            showModal(
                modalDialog(
                    "Please remove your selection before going back.",
                    size = "s",
                   ...
                   !is.null(input$select2_rows_selected)) {
            showModal(
                modalDialog(
                    "Please remove your selection before going back.",
                    size = "s",
    ...
    output$btns <- renderUI({
        # This stops the function if data isn't ready
        req(values$data)
        
        if (values$data == 1) {
            # Return a tagList to ensure clean HTML delivery
            tagList(
                selectizeInput(
                    "item_name",
                    label = "Step 1: Choose a Job",
                    choices = NULL,
                    width = "100%",
                    options = list(
            ...
            DT::dataTableOutput("select2")
            
        } else if (values$data == 3) {
            DT::dataTableOutput("select3")
            
        } else if (values$data == 4) {
            DT::dataTableOutput("select4")
            
        } else if (values$data == 5) {
            DT::dataTableOutput("select5")
            
        }
    })
    
    # Crucial: This tells Shiny NOT to wait for the UI to be visible to render it
    outputOptions(output, "btns", suspendWhenHidden = FALSE)
    ...
    output$searchNeo <- renderUI({
        ...
        )  # when clicked, the link() code executes
    })
    
    # Start Button -------------------------------------------------------------
    observeEvent(input$startBtn, {
        updateNavbarPage(session, "navBar", selected = "careerPF")
    })
    ...
    # This handles the 2,000+ items on the server side
    observe({
        req(item_ref)
    ...
    # Table 1 (Step 2)
    top1 <- reactive({
        # Change the req() to look at the frozen value
        req(values$frozen_job1)
        
        # Use values$frozen_job here:
        top <- item_pairs() %>%
            dplyr::filter(Item1Name == values$frozen_job1) %>%
            dplyr::select(Item2Name,
                          Item2,
                          Prob,
                          Salary2Min,
                          SalaryDiff,
    ...
    output$select2 <- DT::renderDataTable({
        ...
        datatable(
            d,
            escape = FALSE,
            # 1. Remove 'Responsive' for now to ensure it doesn't hide columns
            ...
            options = list(
                width = "100%",
                scrollX = TRUE,
                # 2. Use horizontal scrolling instead
                autoWidth = TRUE
            )
        ) %>%
            formatCurrency(5) %>%
    ...
    # Table 2 (Step 3)
    top2 <- reactive({
        # Look at the 'frozen' selection from the previous step
        req(values$frozen_job2)
        
        dplyr::filter(item_pairs(), Item1Name == values$frozen_job2) %>%
            select(Item2Name,
                   Item2,
                   Prob,
                   Salary2Min,
    ...
    # Table 3 (Step 4)
    top3 <- reactive({
        req(values$frozen_job3)
        
        dplyr::filter(item_pairs(), Item1Name == values$frozen_job3) %>%
            select(Item2Name,
                   Item2,
                   Prob,
                   Salary2Min,
    ...
    # Table 4 (Step 5)
    top4 <- reactive({
        req(values$frozen_job4)
        
        dplyr::filter(item_pairs(), Item1Name == values$frozen_job4) %>%
            select(Item2Name,
                   Item2,
                   Prob,
                   Salary2Min,
    ...
    output$stepNo <- renderUI({
        if (values$data == 1) {
            ...
            # tags$h4("Step 2:")
            div(tags$h4("Step 2:"), div(
                tags$h6("Choose from one of the jobs in the table below")
            ))
        } else if (values$data == 3) {
            tags$h4("Step 3:")
        } else if (values$data == 4) {
    ...
    output$printInput1 <- renderUI({
        ...
        if (input$item_name == "") {
            ...
            div(class = "panel panel-default", div(class = "panel-body", div(
                ...
                tags$h6(paste0(
                    input$item_name, " (", data[2], ")"
                )),
                paste0(data[3], " - ", data[4], " /month"),
                div(paste0(data[5], " incumbents"))
            )))
        }
    })
    ...
    output$pdflink <- downloadHandler(filename <- "my-career.pdf", content <- function(file) {
    ...
    onclick("download", delay(5000, c(
        showModal(
            modalDialog(
                ...
                tags$a(
                    href = "https://survey.lacounty.gov/LACounty/se.ashx?s=645F266E52CB09F0",
                    target = "_blank",
